...
internal sealed class RuntimeStore
{
  ...
  public RuntimeState LoadAndRepair()
  {
    ...
    if (!File.Exists(statePath))
      throw new InvalidDataException("EnhancedCountdown Runtime/state.json is missing. Install version 0.2.0 manually.");

    RuntimeState state =
      JsonConvert.DeserializeObject<RuntimeState>(File.ReadAllText(statePath))
      ?? throw new InvalidDataException("EnhancedCountdown runtime state is empty.");
    if (state.SchemaVersion != 2 || string.IsNullOrWhiteSpace(state.Current))
      throw new InvalidDataException("EnhancedCountdown runtime state is invalid.");
  ...
  public RuntimeCandidate ValidateCandidate(string version, string runtimePath)
  {
    ...
    string assembly = Path.Combine(actual, "EnhancedCountdown.dll");
    string engine = Path.Combine(actual, "EnhancedCountdown.UpdateEngine.dll");
  ...
  {
    TryDeleteFile(Path.Combine(installPath, "EnhancedCountdown.dll"));
    TryDeleteFile(Path.Combine(installPath, "EnhancedCountdown.pdb"));
    TryDeleteFile(Path.Combine(installPath, "EnhancedCountdown.deps.json"));
  }
