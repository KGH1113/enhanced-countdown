...
using RemoveCountdown.Launcher;
using RemoveCountdown.UpdateEngine;
...
internal static class Program
{
  ...
  private static void TestPackageInstallation()
  {
    ...
    Assert(File.Exists(Path.Combine(result.RuntimePath, "RemoveCountdown.dll")), "Payload was not installed.");
    Assert(
      File.Exists(Path.Combine(result.RuntimePath, "RemoveCountdown.UpdateEngine.dll")),
  ...
  private static byte[] CreatePackage(string version, bool includeUnsafeEntry)
  {
    ...
    {
      string root = $"RemoveCountdown/Runtime/versions/{version}/";
      AddEntry(archive, root + "RemoveCountdown.dll", "payload");
      AddEntry(archive, root + "RemoveCountdown.UpdateEngine.dll", "engine");
  ...
  {
    return $"{{\"schemaVersion\":1,\"version\":\"{version}\",\"packageAsset\":\"RemoveCountdown.zip\",\"packageBytes\":{bytes},\"packageSha256\":\"{checksum}\",\"runtimePath\":\"RemoveCountdown/Runtime/versions/{version}\"}}";
  }
  ...
  private static void CreateRuntime(string installPath, string version)
  {
    ...
    File.WriteAllText(Path.Combine(runtime, "RemoveCountdown.dll"), "payload");
    File.WriteAllText(Path.Combine(runtime, "RemoveCountdown.UpdateEngine.dll"), "engine");
...
internal sealed class TemporaryDirectory : IDisposable
{
  public TemporaryDirectory()
  {
    Path = System.IO.Path.Combine(
      ...
      "remove-countdown-tests-" + Guid.NewGuid().ToString("N")
    );
