...
{
  private const string PayloadEntryMethod = "EnhancedCountdown.Main.Load";
  ...
  private static bool TryLoadCurrent(UnityModManager.ModEntry modEntry, RuntimeCandidate current)
  {
    ...
    if (TryLoad(modEntry, current, out Exception exception))
      return true;
    modEntry.Logger.Error(exception?.ToString() ?? "The current EnhancedCountdown runtime failed to load.");
    return false;
  }
